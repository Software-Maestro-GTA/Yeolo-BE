...
class UserControllerTest {
    ...
    /**
     * 탈퇴 후에도 Access Token은 수명(기본 1시간)만큼 서명·만료가 유효하다. 탈퇴 시 Refresh만
     * 지우면 그 창 동안 보호 API를 계속 쓸 수 있으므로, 인증 단계에서 계정 상태로 막아야 한다.
     */
    @Test
    void 탈퇴한_사용자의_남은_토큰은_401로_거부한다() throws Exception {
        UUID userId = UUID.randomUUID();
        when(jwtTokenProvider.parseAccessTokenUserId("valid-token")).thenReturn(userId);
        when(withdrawnUserChecker.isWithdrawn(userId)).thenReturn(true);
        ...
        verify(userWithdrawalService, never()).withdraw(userId);
    }
